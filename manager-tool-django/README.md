...
## Render deploy (Phase 4)

The `render.yaml` at the repo root defines the web service declaratively. First-time setup:

1. Sign in to [Render](https://render.com) (free) → **New → Blueprint**
2. Connect the GitHub repo (`toddaerickson/manager-tool`); Render auto-detects `render.yaml`
3. Review the service plan: `starter` ($7/mo, no cold starts — recommended per `MIGRATION_PLAN.md` Phase 4)
4. Paste the secret env vars when prompted:
   - `DATABASE_URL` — Neon `dev-django` branch URL
   - `CONFIG_ENCRYPTION_KEY` — Fernet key
   - `SENTRY_DSN` — same DSN used locally
   - `GOOGLE_OAUTH_CLIENT_ID` and `GOOGLE_OAUTH_CLIENT_SECRET` — same client used locally
   - (`DJANGO_SECRET_KEY` auto-generated by Render via `generateValue: true`)
5. Render builds + starts the service. URL: `https://manager-tool-django.onrender.com` (or whatever Render assigns — check the dashboard)

After first successful deploy:
6. Add the Render URL as a second **Authorized redirect URI** in Google Cloud Console for the OAuth client:
   `https://manager-tool-django.onrender.com/accounts/google/login/callback/`
   (one OAuth client, two URIs: localhost for dev, Render hostname for prod)
7. Hit `https://<render-host>/sentry-debug/` to verify Sentry receives the test exception within 60s (Phase 4 → 5 gate)
